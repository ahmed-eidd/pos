...
const DiscountInputs = ({ isDiscount, setIsDiscount, discountType, discountValue, onDiscountValueChange, onDiscoutTypeChange, password, setPassword }) => {
  ...
  `;
  return (
    <Flex style={{ margin: '1rem 0' }} direction="column" align="flex-start" justify="center" gap="15px" className={DiscountInputsStyles}>
      <Flex direction="row" justify="flex-start" align="center" gap="10px">
        <Text label weight="bold">
          تخفيض:
        </Text>
        <Checkbox checked={isDiscount} onChange={e => setIsDiscount(e.target.checked)} />
      </Flex>
      {isDiscount && (
        <>
          <Radio.Group onChange={event => onDiscoutTypeChange(event)} value={discountType}>
            <Flex gap="10px">
